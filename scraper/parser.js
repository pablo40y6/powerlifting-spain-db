...
function parseGoodliftDetailedScoresheetLine(line, competition, sex, fallbackCategory) {
  ...
  return makeAthleteEntry({
    competition,
    sex,
    category: fallbackCategory || inferPowerliftingCategoryFromBodyweight(bodyweight, sex),
...
async function parsePdfDocument(buffer, baseMeta) {
  ...
  if (repairedEntries.length || !isGoodliftPdfText(rawText)) return repairedEntries;
