...
function hasValidAttemptInMovement(attempts, movement) {
  return (attempts?.[movement] || []).some((attempt) =>
    attempt && attempt.good === true && attempt.weight !== null && attempt.weight !== undefined
  );
}

function hasValidPowerliftingTotal(attempts, eventType) {
  if ((eventType || 'powerlifting') !== 'powerlifting') return true;
  return ['squat', 'bench', 'deadlift'].every((movement) =>
    hasValidAttemptInMovement(attempts, movement)
  );
}
...
function makeAthleteEntry({
  ...
  return {
    ...
    isIndividualResult: Boolean(isIndividualResult),
...
function parseExcelDocument(buffer, baseMeta) {
  ...
  for (let sheetIndex = 0; sheetIndex < workbook.SheetNames.length; sheetIndex += 1) {
    ...
    for (let index = 0; index < sheetRows.length; index += 1) {
      ...
      if (!shouldRejectNonAthleteResult(entry)) entries.push(entry);
    }
  }
...
function sexFromText(text) {
  const normalized = normalizeName(text);
  if (/\b(mujeres|femenino|women|female)\b/.test(normalized)) return 'F';
  if (/\b(hombres|masculino|men|male)\b/.test(normalized)) return 'M';
  return null;
}

function isGoodliftPdfText(text) {
  return /Generated by the GOODLIFT|DETAILED SCORESHEET/i.test(String(text || ''));
}
...
function athleteNameHasClubShape(name) {
  ...
  if (/barbell/.test(normalized)) return true;
  ...
  const hasTeamSeparators = /[-'’]/.test(raw);
...
function athleteNameLooksLikeClubOrTeam(name, options = {}) {
  ...
  if (teamWords.test(normalized) || /barbell/.test(normalized)) return true;
...
function entryHasAnyAttemptWeight(entry) {
  return ['squat', 'bench', 'deadlift'].some((movement) =>
    (entry?.attempts?.[movement] || []).some((attempt) => attempt && attempt.weight !== null && attempt.weight !== undefined)
  );
}
...
function shouldRejectNonAthleteResult(entry) {
  const athleteName = entry?.athleteName ?? entry?.lifterName ?? '';
  const club = entry?.club ?? '';
  if (hasTeamPointsFormula(athleteName) || hasTeamPointsFormula(club)) return true;
...
function inferPowerliftingCategoryFromBodyweight(bodyweight, sex) {
  if (bodyweight === null || bodyweight === undefined || bodyweight <= 0) return null;

  const limits = sex === 'F'
    ? [43, 47, 52, 57, 63, 69, 76, 84]
    : [53, 59, 66, 74, 83, 93, 105, 120];

  for (const limit of limits) {
    if (bodyweight <= limit + 0.01) return `-${limit}kg`;
  }

  return sex === 'F' ? '+84kg' : '+120kg';
}

function parseGoodliftAttempt(raw) {
  ...
  if (!text || text === '-' || /^X$/i.test(text)) {
    return { raw: /^X$/i.test(text) ? text : null, weight: null, good: null };
  }
...
function goodliftAttemptsObject({ squatTokens, benchTokens, deadliftTokens }) {
  return {
    squat: squatTokens.map(parseGoodliftAttempt),
    bench: benchTokens.map(parseGoodliftAttempt),
    deadlift: deadliftTokens.map(parseGoodliftAttempt),
    __labels: { ...DEFAULT_LIFT_LABELS },
  };
}
...
function parseGoodliftDetailedScoresheetLine(line, competition, sex, fallbackCategory) {
  ...
  const squatRank = parseLocaleNumber(stats[11]);
  const movementRanks = {};
  ...
  const attempts = goodliftAttemptsObject({
    squatTokens: [stats[14], stats[13], stats[12]],
    benchTokens: [stats[10], stats[9], stats[8]],
    deadliftTokens: [stats[6], stats[5], stats[4]],
  });

  return makeAthleteEntry({
    competition,
    sex,
    category: inferPowerliftingCategoryFromBodyweight(bodyweight, sex) || fallbackCategory,
    placing: placingToken,
    lifterName,
    yearOfBirth: Number(tokens[yearIndex]),
    club,
    bodyweight,
    coefficient,
    order,
    ...
    movementRanks: Object.keys(movementRanks).length ? movementRanks : null,
  });
}
...
function parsePdfAthleteLineWithoutYear(tokens, competition, sex, fallbackCategory, liftType = 'powerlifting') {
  ...
  return makeAthleteEntry({
    competition,
    sex,
    category: rowCategory || inferPowerliftingCategoryFromBodyweight(bodyweightOrder.bodyweight, sex) || fallbackCategory,
    placing,
...
function mergePreferredEntry(preferred, fallback) {
  ...
  const eventType = pickValue(preferred.eventType, fallback.eventType) || pickValue(preferred.liftType, fallback.liftType) || 'powerlifting';
  const validPowerliftingTotal = hasValidPowerliftingTotal(selectedAttempts, eventType);
  const total = pickValue(preferred.total, fallback.total);
  const isIndividualResult = preferred.isIndividualResult !== false && fallback.isIndividualResult !== false;
  ...
  return {
    ...
    isIndividualResult,
    hasValidPowerliftingTotal: validPowerliftingTotal,
    isRankable: Boolean(isIndividualResult && total !== null && total !== undefined && total > 0 && validPowerliftingTotal),
    competition: {
...
function mergeAthleteEntries(entries) {
  ...
  for (const entry of entries) {
    if (shouldRejectNonAthleteResult(entry)) continue;
